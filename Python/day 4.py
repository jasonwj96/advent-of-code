...
suddenly
appears above you! The gondola carries you to the surface of the new island and lurches into the
station.
...
though - er,
the small kind surrounded by water, not the floating kind. We really need to come up with a better
naming scheme. Tell you what: if you can help me with something quick, I'll let you borrow my boat
and you can go visit the gardener. I got all these scratchcards as a gift, but I can't figure out
what I've won."
...
with open('input/input_4.txt') as file:
    ...
    for index, line in enumerate(lines):
        ...
        match_count = len(set(card_nums) & set(rest_nums))

        if match_count > 0:
            count += 2 ** (match_count - 1)

        for number in range(match_count):
            bonus_points[index + number + 1] += bonus_points[index]

print(f"Part 1: {count}")
print(f"Part 2: {sum(bonus_points.values())}")

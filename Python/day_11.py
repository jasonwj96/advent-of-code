...
project. However,
he confirms that the hot springs are the next-closest area likely to have people; he'll even take
you straight there
...
replaced with 1000000
empty columns.

(In the example above, if each empty row or column were merely 10 times larger, the sum of the
shortest paths between
every pair of galaxies would be 1030. If each empty row or column were merely 100 times larger,
the sum of the
shortest paths between every pair of galaxies would be 8410. However, your universe will need to
expand far beyond
these values.)
...
with open("input/input_11.txt") as file:
    ...
    for j, _ in enumerate(lines[0]):
        if all(lines[i][j] != "#" for i, _ in enumerate(lines)):
            empty_c.append(j)
    ...
    distances = squareform(pdist(coordinates, "cityblock"))

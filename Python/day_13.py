...
around.
After a while, you make your way to a nearby cluster of mountains only to discover that the
valley between
them is completely full of large mirrors. Most of the mirrors seem to be aligned in a consistent
way;
...
large metal
frames keeping them in place. The mirrors are extremely flat and shiny, and many of the fallen
mirrors have
lodged into the ash at strange angles. Because the terrain is all one color, it's hard to tell
where it's
...
vertical
line is not perfectly in the middle of the pattern, part of the pattern (column 1) has nowhere to
reflect
onto and can be ignored; every other column has a reflected column within the pattern and must
match
...
hypothetical
row 8, but since that's not in the pattern, row 1 doesn't need to match anything.
...
"""
"""--- Part Two --- You resume walking through the valley of mirrors and - SMACK! - run directly 
into one. Hopefully nobody was watching, because that must have been pretty embarrassing.
...
#...##..# #....#..# ..##..### #####.##. #####.##. ..##..### #....#..# The first pattern's smudge 
is in the top-left corner. If the top-left # were instead ., it would have a different, 
    ...
    for offset in range(len(pattern) - 1):
        ...
        for index in range(len(pattern)):
            ...
            if index2 in range(len(pattern)) and pattern[index1] != pattern[index2]:
                total_smudge += len(
                    [cell for cell in range(len(pattern[index1])) if
                     pattern[index1][cell] != pattern[index2][cell]])

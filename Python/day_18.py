...
directions are given as seen from above, so if "up" were north, then "right" would be east,
and so on. Each trench is also listed with the color that the edge of the trench should be
...
def part1():
    ...
    for line in open('input/input_18.txt'):
        d, n, _ = line.split()
        dr, dc = dirs[d]
        n = int(n)
        b += n
    ...
    A = abs(sum(points[i][0] * (points[i - 1][1] - points[(i + 1) % len(points)][1]) for i in
                range(len(points)))) // 2
    i = A - b // 2 + 1
